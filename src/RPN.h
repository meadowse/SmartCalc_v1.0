...
Выталкивает из стэка в конечную очередь все операции с таким же или большим
приоритетом, если они находятся в голове стэка.
 */
void restack_binary(int id, Stack *stack, Queue *out);

// Функция переводящая операцию из символа ('+', '-' и тд.) в числовой
// эквивалент
...
*is_unary - указатель на переменную, храняющую является ли ближайший минус
унарным *que - указатель на выходную очередь, в которую будут записаны лексемы
Возврат:
